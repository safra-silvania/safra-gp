                ...
                <table class="table mb-none">
                    <thead>
                        <tr>
                            ...
                            <th><?= $this->Paginator->sort('document', __('Document')) ?></th>
                            <th><?= $this->Paginator->sort('phone_cel', __('Phone_cel')) ?></th>
                            <th><?= $this->Paginator->sort('phone_fix', __('Phone_fix')) ?></th>
                    ...
                    <tbody>
                        <?php foreach ($producers as $producer): ?>
                            <tr>
                                ...
                                <td><?= h($producer->document) ?></td>
                                <td><?= h($producer->phone_cel) ?></td>
                                <td><?= h($producer->phone_fix) ?></td>
                                <td><?= $producer->has('city') ? ($userAuthenticated->can('view', $producer->city) ? $this->Html->link($producer->city->name, ['controller' => 'Cities', 'action' => 'view', $producer->city->id]) : $producer->city->name) : '' ?></td>
                                <td><?= h($producer->created) ?></td>
                                <td><?= h($producer->modified) ?></td>
                                <td class="actions">
                                    <?php
                                    if ($userAuthenticated && $userAuthenticated->can('view', $producer)) {
                                        echo $this->Html->link(
                                            '<button type="button" class="mr-xs mb-xs btn btn-xs btn-info"><i class="fa fa-search"></i></button>',
                                            ['action' => 'view', $producer->id],
                                            ['escape' => false, 'title' => 'Visualizar']
                                        );
                                    }

                                    if ($userAuthenticated && $userAuthenticated->can('update', $producer)) {
                                        echo $this->Html->link(
                                            '<button type="button" class="mr-xs mb-xs btn btn-xs btn-warning"><i class="fa fa-pencil"></i></button>',
                                            ['action' => 'edit', $producer->id],
                                            ['escape' => false, 'title' => 'Editar']
                                        );
                                    }
                                        
                                    if ($userAuthenticated && $userAuthenticated->can('delete', $producer)) {

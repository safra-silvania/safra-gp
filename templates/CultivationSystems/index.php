 ...
 * @var \App\Model\Entity\CultivationSystem[]|\Cake\Collection\CollectionInterface $cultivationSystems
 */
?>
        ...
        <div class="panel-body">
            <div class="table-responsive">
                <table class="table mb-none">
                    ...
                    <tbody>
                        <?php foreach ($cultivationSystems as $cultivationSystem): ?>
                            <tr>
                                <td><?= $this->Number->format($cultivationSystem->id) ?></td>
                                <td><?= h($cultivationSystem->name) ?></td>
                                <td><?= h($cultivationSystem->created) ?></td>
                                <td><?= h($cultivationSystem->modified) ?></td>
                                <td class="actions">
                                    <?php
                                    if ($userAuthenticated && $userAuthenticated->can('view', $cultivationSystem)) {
                                        echo $this->Html->link(
                                            '<button type="button" class="mr-xs mb-xs btn btn-xs btn-info"><i class="fa fa-search"></i></button>',
                                            ['action' => 'view', $cultivationSystem->id],
                                            ['escape' => false, 'title' => 'Visualizar']
                                        );
                                    }

                                    if ($userAuthenticated && $userAuthenticated->can('update', $cultivationSystem)) {
                                        echo $this->Html->link(
                                            '<button type="button" class="mr-xs mb-xs btn btn-xs btn-warning"><i class="fa fa-pencil"></i></button>',
                                            ['action' => 'edit', $cultivationSystem->id],
                                            ['escape' => false, 'title' => 'Editar']
                                        );
                                    }
                                        
                                    if ($userAuthenticated && $userAuthenticated->can('delete', $cultivationSystem)) {
                                        echo $this->Form->postLink(
                                            '<button type="button" class="mr-xs mb-xs btn btn-xs btn-danger"><i class="fa fa-trash-o"></i></button>',
                                            ['action' => 'delete', $cultivationSystem->id],

 ...
 * @var \App\Model\Entity\ChemicalClass[]|\Cake\Collection\CollectionInterface $chemicalClasses
 */
?>
                ...
                <table class="table mb-none">
                    ...
                    <tbody>
                        <?php foreach ($chemicalClasses as $chemicalClass): ?>
                            <tr>
                                <td><?= $this->Number->format($chemicalClass->id) ?></td>
                                <td><?= h($chemicalClass->name) ?></td>
                                <td><?= h($chemicalClass->created) ?></td>
                                <td><?= h($chemicalClass->modified) ?></td>
                                <td class="actions">
                                    <?php
                                    if ($userAuthenticated && $userAuthenticated->can('view', $chemicalClass)) {
                                        echo $this->Html->link(
                                            '<button type="button" class="mr-xs mb-xs btn btn-xs btn-info"><i class="fa fa-search"></i></button>',
                                            ['action' => 'view', $chemicalClass->id],
                                            ['escape' => false, 'title' => 'Visualizar']
                                        );
                                    }

                                    if ($userAuthenticated && $userAuthenticated->can('update', $chemicalClass)) {
                                        echo $this->Html->link(
                                            '<button type="button" class="mr-xs mb-xs btn btn-xs btn-warning"><i class="fa fa-pencil"></i></button>',
                                            ['action' => 'edit', $chemicalClass->id],
                                            ['escape' => false, 'title' => 'Editar']
                                        );
                                    }
                                        
                                    if ($userAuthenticated && $userAuthenticated->can('delete', $chemicalClass)) {
                                        echo $this->Form->postLink(
                                            '<button type="button" class="mr-xs mb-xs btn btn-xs btn-danger"><i class="fa fa-trash-o"></i></button>',
                                            ['action' => 'delete', $chemicalClass->id],

 ...
 * @var \App\Model\Entity\ApplicationSeason[]|\Cake\Collection\CollectionInterface $applicationSeasons
 */
?>
        ...
        <div class="panel-body">
            <div class="table-responsive">
                <table class="table mb-none">
                    ...
                    <tbody>
                        <?php foreach ($applicationSeasons as $applicationSeason): ?>
                            <tr>
                                <td><?= $this->Number->format($applicationSeason->id) ?></td>
                                <td><?= h($applicationSeason->name) ?></td>
                                <td><?= h($applicationSeason->created) ?></td>
                                <td><?= h($applicationSeason->modified) ?></td>
                                <td class="actions">
                                    <?php
                                    if ($userAuthenticated && $userAuthenticated->can('view', $applicationSeason)) {
                                        echo $this->Html->link(
                                            '<button type="button" class="mr-xs mb-xs btn btn-xs btn-info"><i class="fa fa-search"></i></button>',
                                            ['action' => 'view', $applicationSeason->id],
                                            ['escape' => false, 'title' => 'Visualizar']
                                        );
                                    }

                                    if ($userAuthenticated && $userAuthenticated->can('update', $applicationSeason)) {
                                        echo $this->Html->link(
                                            '<button type="button" class="mr-xs mb-xs btn btn-xs btn-warning"><i class="fa fa-pencil"></i></button>',
                                            ['action' => 'edit', $applicationSeason->id],
                                            ['escape' => false, 'title' => 'Editar']
                                        );
                                    }
                                        
                                    if ($userAuthenticated && $userAuthenticated->can('delete', $applicationSeason)) {
                                        echo $this->Form->postLink(
                                            '<button type="button" class="mr-xs mb-xs btn btn-xs btn-danger"><i class="fa fa-trash-o"></i></button>',
                                            ['action' => 'delete', $applicationSeason->id],

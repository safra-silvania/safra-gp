 ...
 * @var \App\Model\Entity\ChemicalGroup[]|\Cake\Collection\CollectionInterface $chemicalGroups
 */
?>
        ...
        <div class="panel-body">
            <div class="table-responsive">
                <table class="table mb-none">
                    ...
                    <tbody>
                        <?php foreach ($chemicalGroups as $chemicalGroup): ?>
                            <tr>
                                <td><?= $this->Number->format($chemicalGroup->id) ?></td>
                                <td><?= h($chemicalGroup->name) ?></td>
                                <td><?= h($chemicalGroup->created) ?></td>
                                <td><?= h($chemicalGroup->modified) ?></td>
                                <td class="actions">
                                    <?php
                                    if ($userAuthenticated && $userAuthenticated->can('view', $chemicalGroup)) {
                                        echo $this->Html->link(
                                            '<button type="button" class="mr-xs mb-xs btn btn-xs btn-info"><i class="fa fa-search"></i></button>',
                                            ['action' => 'view', $chemicalGroup->id],
                                            ['escape' => false, 'title' => 'Visualizar']
                                        );
                                    }

                                    if ($userAuthenticated && $userAuthenticated->can('update', $chemicalGroup)) {
                                        echo $this->Html->link(
                                            '<button type="button" class="mr-xs mb-xs btn btn-xs btn-warning"><i class="fa fa-pencil"></i></button>',
                                            ['action' => 'edit', $chemicalGroup->id],
                                            ['escape' => false, 'title' => 'Editar']
                                        );
                                    }
                                        
                                    if ($userAuthenticated && $userAuthenticated->can('delete', $chemicalGroup)) {
                                        echo $this->Form->postLink(
                                            '<button type="button" class="mr-xs mb-xs btn btn-xs btn-danger"><i class="fa fa-trash-o"></i></button>',
                                            ['action' => 'delete', $chemicalGroup->id],

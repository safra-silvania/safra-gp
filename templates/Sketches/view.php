            ...
            <table class="table table-hover">
                <tbody>
                    <tr>
                        ...
                        <td class="col-lg-9"><?= $sketch->has('field') ? ($userAuthenticated->can('view', $sketch->field) ? $this->Html->link($sketch->field->name, ['controller' => 'Fields', 'action' => 'view', $sketch->field->id]) : $sketch->field->name) : '' ?></td>
                    </tr>
                    <tr>
                        <th class="col-lg-3"><?= __('Id') ?></th>
                        <td class="col-lg-9"><?= $this->Number->format($sketch->id) ?></td>
                    </tr>
                    <tr>
                        <th class="col-lg-3"><?= __('Created') ?></th>
                        <td class="col-lg-9"><?= h($sketch->created) ?></td>
                    </tr>
                    <tr>
                        <th class="col-lg-3"><?= __('Modified') ?></th>
                        <td class="col-lg-9"><?= h($sketch->modified) ?></td>
                    </tr>
                </tbody>
            </table>
            <div class="text">
                ...
                <blockquote>
                    <?= $this->Text->autoParagraph(h($sketch->observations)); ?>
                </blockquote>
            </div>
        </div>
    </section>
    ...
    <section class="panel panel-collapsed">
        <header class="panel-heading">
            <div class="panel-actions">
                <a href="#" id="view-audit-log" class="panel-action panel-action-toggle" data-panel-toggle="" data-target-url="<?= Cake\Routing\Router::url(["controller"=>"AuditLogs","action"=>"get-timeline-data", $sketch->id, 'sketches']); ?>"></a>
            </div>

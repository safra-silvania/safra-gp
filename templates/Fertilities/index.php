 ...
 * @var \App\Model\Entity\Fertility[]|\Cake\Collection\CollectionInterface $fertilities
 */
?>
        ...
        <div class="panel-body">
            <div class="table-responsive">
                <table class="table mb-none">
                    ...
                    <tbody>
                        <?php foreach ($fertilities as $fertility): ?>
                            <tr>
                                <td><?= $this->Number->format($fertility->id) ?></td>
                                <td><?= h($fertility->name) ?></td>
                                <td><?= h($fertility->created) ?></td>
                                <td><?= h($fertility->modified) ?></td>
                                <td class="actions">
                                    <?php
                                    if ($userAuthenticated && $userAuthenticated->can('view', $fertility)) {
                                        echo $this->Html->link(
                                            '<button type="button" class="mr-xs mb-xs btn btn-xs btn-info"><i class="fa fa-search"></i></button>',
                                            ['action' => 'view', $fertility->id],
                                            ['escape' => false, 'title' => 'Visualizar']
                                        );
                                    }

                                    if ($userAuthenticated && $userAuthenticated->can('update', $fertility)) {
                                        echo $this->Html->link(
                                            '<button type="button" class="mr-xs mb-xs btn btn-xs btn-warning"><i class="fa fa-pencil"></i></button>',
                                            ['action' => 'edit', $fertility->id],
                                            ['escape' => false, 'title' => 'Editar']
                                        );
                                    }
                                        
                                    if ($userAuthenticated && $userAuthenticated->can('delete', $fertility)) {
                                        echo $this->Form->postLink(
                                            '<button type="button" class="mr-xs mb-xs btn btn-xs btn-danger"><i class="fa fa-trash-o"></i></button>',
                                            ['action' => 'delete', $fertility->id],

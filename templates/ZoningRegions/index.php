 ...
 * @var \App\Model\Entity\ZoningRegion[]|\Cake\Collection\CollectionInterface $zoningRegions
 */
?>
        ...
        <div class="panel-body">
            <div class="table-responsive">
                <table class="table mb-none">
                    ...
                    <tbody>
                        <?php foreach ($zoningRegions as $zoningRegion): ?>
                            <tr>
                                <td><?= $this->Number->format($zoningRegion->id) ?></td>
                                <td><?= h($zoningRegion->name) ?></td>
                                <td class="actions">
                                    <?php
                                    if ($userAuthenticated && $userAuthenticated->can('view', $zoningRegion)) {
                                        echo $this->Html->link(
                                            '<button type="button" class="mr-xs mb-xs btn btn-xs btn-info"><i class="fa fa-search"></i></button>',
                                            ['action' => 'view', $zoningRegion->id],
                                            ['escape' => false, 'title' => 'Visualizar']
                                        );
                                    }

                                    if ($userAuthenticated && $userAuthenticated->can('update', $zoningRegion)) {
                                        echo $this->Html->link(
                                            '<button type="button" class="mr-xs mb-xs btn btn-xs btn-warning"><i class="fa fa-pencil"></i></button>',
                                            ['action' => 'edit', $zoningRegion->id],
                                            ['escape' => false, 'title' => 'Editar']
                                        );
                                    }
                                        
                                    if ($userAuthenticated && $userAuthenticated->can('delete', $zoningRegion)) {
                                        echo $this->Form->postLink(
                                            '<button type="button" class="mr-xs mb-xs btn btn-xs btn-danger"><i class="fa fa-trash-o"></i></button>',
                                            ['action' => 'delete', $zoningRegion->id],
